...
public class PokemonServiceTest extends TestCase {
    ...
    @Mock
    PokemonRepository mockRepo = Mockito.mock(PokemonRepository.class);
    Trainer tr = Mockito.mock(Trainer.class);

    PokemonService pkServe = new PokemonService(mockRepo);

    @Before
    public void setUpTests(){
        Mockito.when(mockRepo.updatePokemon(Mockito.any())).thenReturn(true);
    }

    public void testGetNewPokemon() {
        List<Pokemon> mockList = new ArrayList<Pokemon>();
